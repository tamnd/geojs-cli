...
describes what users get. Pick a format with `-o`, or let geojs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
geojs <command> -o table   # aligned columns for reading
geojs <command> -o jsonl   # one JSON object per line, for piping
geojs <command> -o json    # a single JSON array
geojs <command> -o csv     # spreadsheet friendly
geojs <command> -o tsv     # tab-separated
geojs <command> -o url     # just the URL column
geojs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
geojs <command> --fields id,title,url
```
...
```bash
geojs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
geojs <command>            # a table, because this is a terminal
geojs <command> | wc -l    # JSONL, because this is a pipe
```
